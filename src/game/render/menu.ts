...
import {
  MODE_INFO, getDailyModifierLabel, getDailySeed, pickDailyModifier,
} from '../modes';
import {
  isAircraftUnlocked, isWeaponUnlocked, canAffordUnlock, loadCoins,
} from '../progress';

export function drawMenu(ctx: CanvasRenderingContext2D, g: GameData) {
  ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
  ctx.textAlign = 'center';

  ctx.save(); ctx.shadowColor = '#0af'; ctx.shadowBlur = 40;
  ctx.fillStyle = '#fff'; ctx.font = 'bold 42px "Segoe UI",Arial,sans-serif';
  ctx.fillText('SKY BLASTER', CANVAS_W / 2, 130); ctx.restore();
  ...
  if (g.gameMode === 'daily') {
    ctx.fillStyle = '#da8';
    ctx.fillText(`Today: ${getDailyModifierLabel(pickDailyModifier(getDailySeed()))}`, CANVAS_W / 2, 236);
  }
  ...
  ctx.fillText(`◀  ${craft.name.toUpperCase()}${craftUnlocked ? '' : ' 🔒'}  ▶`, CANVAS_W / 2, 286);
  ...
  ctx.fillText(`◀  ${weapon.name.toUpperCase()}${weaponUnlocked ? '' : ' 🔒'}  ▶`, CANVAS_W / 2, 362);
  if (!weaponUnlocked) {
  ...
  lines.forEach(([k, v], i) => {
    ...
    ctx.fillStyle = '#aac'; ctx.font = '11px "Segoe UI",Arial,sans-serif';
    ctx.textAlign = 'right'; ctx.fillText(k, CANVAS_W / 2 - 8, y);
    ctx.fillStyle = '#88a'; ctx.textAlign = 'left'; ctx.fillText(v, CANVAS_W / 2 + 8, y);
  });
  ...
  if (sc.length) {
    ...
    sc.slice(0, 3).forEach((s, i) => {
      ...
      ctx.fillText(`${i + 1}. ${String(s.score).padStart(8)}  W${s.wave}  ${s.date}`, CANVAS_W / 2, scoresTop + 16 + i * 15);
    });
  }
}
