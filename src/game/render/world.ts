...
export function drawPlayer(ctx: CanvasRenderingContext2D, g: GameData) {
  ...
  gr2.addColorStop(0, craft.cockpitColor);
  gr2.addColorStop(0.4, craft.engineColor);
  ...
  gr.addColorStop(1, craft.hullBottom);
  ctx.fillStyle = gr;
  ctx.beginPath();
  ...
  ctx.beginPath();
  ctx.moveTo(-4, 0);
  ctx.lineTo(-p.width / 2 + 2, p.height / 3 - 2);
  ctx.moveTo(4, 0);
  ctx.lineTo(p.width / 2 - 2, p.height / 3 - 2);
  ctx.stroke();
...
export function drawBullet(ctx: CanvasRenderingContext2D, b: Bullet) {
  ...
  if (b.isPlayer) {
    ...
    ctx.fillStyle = b.color;
    ctx.beginPath();
    ctx.arc(0, 0, b.width / 2 + 3, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    ctx.fillStyle = b.color;
    ctx.beginPath();
    ctx.arc(0, 0, b.width / 2 + 1, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.beginPath();
