...
export function drawDangerVignette(ctx: CanvasRenderingContext2D, g: GameData) {
  if (g.dangerAlpha > 0.01) {
    const vg = ctx.createRadialGradient(CANVAS_W / 2, CANVAS_H / 2, CANVAS_H * 0.3, CANVAS_W / 2, CANVAS_H / 2, CANVAS_H * 0.7);
...
export function drawGrazeIndicator(ctx: CanvasRenderingContext2D, g: GameData) {
  if (g.player.grazeTimer > 0) {
    ...
    ctx.beginPath(); ctx.arc(g.player.x, g.player.y, GRAZE_RADIUS, 0, Math.PI * 2); ctx.stroke();
    ctx.globalAlpha = 1;
  }
}
